...
export function MembersMarquee() {
  ...
  const [titulaires] = useState<StoredMember[]>(() =>
    getActiveMembers().filter((m) => m.category === "titulaire")
  );
              ...
              >
                {member.logoUrl ? (
                  <div className="flex h-8 w-8 shrink-0 items-center justify-center rounded-lg bg-white border border-white/30 overflow-hidden p-0.5">
                    <Image src={member.logoUrl} alt={member.name} width={32} height={32} className="w-full h-full object-contain" />
                  </div>
                ) : (
