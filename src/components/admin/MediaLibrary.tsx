...
export function MediaLibrary({ open, onClose, onSelect }: MediaLibraryProps) {
  ...
  async function handleDelete(id: string, name: string) {
    if (!window.confirm(`Supprimer le fichier « ${name} » de la Media Library ? Cette action est irréversible.`)) {
      return;
    }
    if (isApiMode()) {
                ...
                >
                  ...
                  <div className="aspect-square bg-[var(--gaspe-neutral-50)] flex items-center justify-center overflow-hidden">
                    {isImage(item.type) && item.data ? (
                      <Image src={item.data} alt={item.alt ?? item.name} width={240} height={240} className="w-full h-full object-cover" unoptimized />
                  ...
                  {/* Delete button — toujours visible (mobile / tactile compris) */}
                  <button
                    onClick={(e) => { e.stopPropagation(); void handleDelete(item.id, item.name); }}
                    className="absolute top-1.5 right-1.5 rounded-lg bg-white/95 p-1.5 shadow-sm border border-[var(--gaspe-neutral-200)] hover:bg-red-50 hover:border-red-300 transition-colors"
