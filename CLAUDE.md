...
- **Version**: v2.32.1 (session 41 Lot 1 : câblage Hydros publication auto sur `/admin/offres/new`. Helper `updateJob(id, partial)` ajouté à `jobs-store.ts` (PATCH côté API, mutation localStorage côté demo). Après `createJob`, fire-and-forget `publishToHydros` quand `published: true` ; `companyDescription` résolue via lookup `members.find(m => m.name === company)` puis fallback générique du `buildHydrosPayload`. Mise à jour silencieuse de `hydrosOfferUrl/Id` via `updateJob` au retour Hydros. v2.32.0 : page admin de fusion `/admin/adherents` qui remplace `/admin/organisations` + `/admin/membres`. Migrations prod 0001-0026 appliquées, smoke tests 26 avr OK : 30 orgs actives, college A=23/B=4/C=3, social3228 True=26/False=3.)
