...
- **Version**: v2.31.0 (session 40, 4 PR mergées #65 → #66 : Vote UX type-spécifique – éditeur d'options multi-items add/remove pour single/multi/ranking + date picker pour date_selection + drag-and-drop natif pour ranking côté adhérent + boutons ↑/↓ a11y conservés. Migrations en prod 0001-0026 toutes appliquées, vérifiées via smoke tests 26 avr post token-fix : 30 orgs actives, college A=23/B=4/C=3, social3228 True=26/False=3, slug TMC reverted, fleet seedée, suppleant 200, jobs.created_by ajouté en 0026.)
