...
**117 pages statiques** générées – deployed on Cloudflare Pages (static export)

## Working copy
- **Repo**: github.com/colombanatsea/gaspe-fr.git
- **Version**: v2.40.0 (session 49 : (1) **Documentation exhaustive** : refonte complète de § 6 « Flux frontend » de `docs/VALIDATION-ANNUELLE-FEATURE.md` (passe de stub session 45 à description détaillée des 5 surfaces livrées sessions 46-47 : banner dynamique, page validation, /admin/campagnes, dashboard, tab démo). § 9 « Évolutions futures » remis à jour avec section « Livré depuis » (manage_validations, diff Y-o-Y, notif J+0) + backlog clair. Nouvelle § 12 « Notification email J+0 ouverture campagne » (~80 lignes : vision, implémentation, template HTML, audience, reste à faire). Footer doc enrichi (sessions 45-49). (2) **Notifications email Brevo `J+0 ouverture campagne`** : helper privé `notifyCampaignOpened(env, campaign)` ajouté à `workers/api.ts` (~140 lignes incluant le template HTML inline). Déclenché depuis `handleCreateCampaign` quand création directe en `status='open'` ET depuis `handleUpdateCampaign` à la transition `draft → open`. **Best-effort** : no-op silencieux si `BREVO_API_KEY` absent (dev local), erreurs Brevo loggées via `console.error` mais ne font pas échouer le PATCH. Audience : tous les `users` avec `is_primary=1`, `archived=0`, `email IS NOT NULL`, joints aux `organizations` actives (`archived=0`). Pas de filtre college / 3228 (la validation concerne toutes les compagnies). Envoi séquentiel (rate-limit safety Brevo). Helper `renderCampaignOpenedEmailHtml(params)` produit un email transactionnel charté ACF (gradient teal `#1B7E8A`, header logo texte + baseline « Localement ancrés. Socialement engagés. », CTA proéminent vers `/espace-adherent/validation`, footer institutionnel) avec sanitization des champs dynamiques (`userName`, `orgName`, `targetDateStr`, `notes`) via `sanitize()`. Bump 2.39.0 → 2.40.0. **Qualité** : lint 0/0, tsc 0, vitest 333/333 (inchangé), build 117 pages OK. **TODO P3 résiduel** : J-14 / J+0 deadline (nécessite Cloudflare Workers Cron Trigger), export PDF d'attestation (lib jsPDF/pdf-lib), smoke tests prod.) v2.39.0 (session 48 : (1) **Permission dédiée `manage_validations`** (11ᵉ permission staff) découplée de `manage_organizations`. `src/lib/auth/types.ts` : ajout dans le type `StaffPermission` + libellé « Campagnes validation annuelle » dans `STAFF_PERMISSION_LABELS`. Worker : 4 endpoints campaigns CRUD + dashboard passent de `manage_organizations` -> `manage_validations`. `canActOnOrg` (helper validation submit + history org-scopée) accepte désormais les deux permissions (rétro-compatibilité : un staff `manage_organizations` reste capable de soumettre une validation cross-org). AdminSidebar item « Campagnes validation » garde sur `manage_validations`. Doc `docs/VALIDATION-ANNUELLE-FEATURE.md` § 3 mis à jour avec le tableau permissions session 48. (2) **Documentation session 47** : ajout d'une § 11 « Diff Y-o-Y » (~80 lignes) dans `docs/VALIDATION-ANNUELLE-FEATURE.md` détaillant le helper pur `diffSnapshots`, ses 14 tests, le composant `<SnapshotDiffModal>`, l'intégration dans le dashboard, et le dictionnaire `FIELD_LABELS_FR`. Footer du doc enrichi : sessions 45-47, 8 helpers, 61 tests unitaires. **Build 117 pages**, lint 0/0, tsc 0, vitest 333/333. Bump 2.38.0 -> 2.39.0.) v2.38.0 (session 47 : **vue diff Y-o-Y** sur le dashboard admin de validation annuelle. (1) Helper pur `diffSnapshots(before, after, options)` ajoute a `workers/handlers/validation-helpers.ts` (~75 lignes) : compare deux snapshots structurellement (deep equality avec sortKeys pour ignorer l'ordre des cles), categorise chaque champ en `added` / `removed` / `modified` / `unchanged`, exclut `id` par defaut, tri canonique modified > added > removed > unchanged puis alphabetique. **+14 tests vitest** (champs identiques, includeUnchanged, modified, added/removed via null, null vs undefined symetriques, nested crewByBrevet, ordre des cles ignore, excludeFields custom, tri par status puis alpha, inputs null gracieux, before/after raw values). **333 tests verts** (319 + 14). (2) Duplication front `src/lib/validation-diff.ts` (workers/ exclu du tsconfig, evite l'import cross-tree) avec dictionnaire `FIELD_LABELS_FR` (15 cles : email, phone, address, websiteUrl, logoUrl, employeeCount, shipCount, name, imo, type, flag, yearBuilt, passengerCapacity, vehicleCapacity, freightCapacity, grossTonnage, crewByBrevet) + helper `formatDiffValue` (string/number/boolean/object stringify). (3) Composant `<SnapshotDiffModal>` (`src/components/validation/`) : overlay `fixed inset-0` avec backdrop z-50, fetch `listValidationsForOrg(slug)` au mount, filtre history par `targetYear` et `targetYear - 1`, paire les items par `(itemType, itemId)`, calcule `diffSnapshots()` par paire, rendu carte par item avec table 4 colonnes (Champ / N-1 / N / Etat) + badges status colores (warm/green/neutral) + valeurs en `font-mono` + valeur N en teal-700. Tri profil > navires alphabetique. Empty states : pas de validation N-1 (nouvel item), pas de validation N (item plus suivi), aucun changement (badge vert). Fermeture par bouton ou touche Echap (a11y). (4) Cablage : nouvelle colonne « Diff Y-o-Y » dans le tableau de `/admin/campagnes/detail?id=X` avec bouton « Voir le diff » par ligne (aria-label francais explicite), state local `diffOrg` declenche le modal, totalChanges affiche dans le header. **Build 117 pages OK**, lint 0/0, tsc 0, vitest **333/333**. Bump 2.37.0 -> 2.38.0.) v2.37.0 (session 46 : **frontend complet de la feature Validation annuelle**. 4 commits sur main (49ff399 → ce-commit). (1) **Banner adhérent** (`src/components/validation/ValidationCampaignBanner.tsx`) : detecte campagne ouverte + items pendings, urgence visuelle teal/orange/red via `deriveUrgency()` (réplique cote front du helper Worker), CTA vers `/espace-adherent/validation`, silencieux en mode démo. Helper `ValidationCampaignBannerForUser({ companyName })` qui résout le slug via `members.find()`. Intégré dans le dashboard `/espace-adherent` sous le bandeau suppléant. (2) **Page `/espace-adherent/validation`** : charge en parallèle campagne courante + historique compagnie + flotte ; carte profil avec form inline éditable (email/phone/address/website/description/employeeCount/shipCount), une carte par navire avec `FleetVesselForm` réutilisé pour édition ; 3 états par item : pending / unchanged / edited ; bouton « Tout marquer Inchangé » pour validation passive en masse ; bandeau sticky en bas avec compteur d'items prêts + submit ; refresh historique après submit, reset état local. Empty state si pas de navires (lien vers `/espace-adherent/flotte`). (3) **Page `/admin/campagnes`** : formulaire création (target_year + target_date + notes + checkbox « Ouvrir immédiatement »), liste cartes par campagne avec badge urgence, boutons d'action contextuels (« Ouvrir » sur draft, « Clôturer » sur open, « Tableau de bord » sur open/closed → query param `id=X`). AdminSidebar : item « Campagnes validation » sous Organisation, gardé par permission `manage_organizations`, icône clipboard-check. (4) **Dashboard `/admin/campagnes/detail?id=X`** : 4 summary cards (compagnies validees / en attente / navires / emails contacts), tableau filtré (Toutes/Pending/Validées) et trié (compagnie/navires/état) avec sort headers `aria-pressed`, barre de progression visuelle par ligne (% navires), bouton **« Relancer N retardataires »** : mailto BCC pré-rempli avec emails titulaires + subject + body en français + lien direct vers `/espace-adherent/validation`, déduplication des emails. (5) **Tab démo** sur `/decouvrir-espace-adherent` : 1 profil + 3 navires (1 validé, 2 pending), bandeau campagne ouverte 2027, barre de progression, boutons disabled (cohérent avec le pattern « démo non-interactive »), CTA adhésion. **Build** : 114 → **117 pages statiques** (+3 routes : `/admin/campagnes`, `/admin/campagnes/detail`, `/espace-adherent/validation`). **Qualité** : lint 0/0, tsc 0, vitest 319/319 (inchangé, pas de nouveaux helpers purs cette session).) v2.36.0 (session 45 : backend complet **Validation annuelle des données adhérents**. 3 nouvelles migrations D1 (0027 `fleet_validation_campaigns`, 0028 ALTER `last_validated_*` sur `organizations` + `organization_vessels`, 0029 `validation_history`) + 6 endpoints Worker (`/api/campaigns` GET/POST + `/api/campaigns/:id` PATCH + `/api/campaigns/:id/dashboard` GET + `/api/organizations/:slug/validations` GET/POST). Section dédiée à la fin de `workers/api.ts` (~640 lignes) ; helpers purs extraits dans `workers/handlers/validation-helpers.ts` (zéro dépendance D1) — 7 fonctions : `isItemValidatedForYear`, `countValidatedItems`, `deriveCampaignUrgency`, `resolveTargetYear`, `buildProfileSnapshot`, `buildVesselSnapshot`, `parseValidationItems`, `summarizeOrgForDashboard`. Vitest config étendu (`workers/**/*.test.ts`) ; **47 nouveaux tests** dans `workers/handlers/__tests__/validation-helpers.test.ts` (edge cases nullables, fenêtres temporelles déterministes via `nowMs` injectable, JSON malformé, doublons items, bornes payload max 200). Cycle campagne `draft → open → closed` **sans auto-fermeture** (admin clôt manuellement) ; `target_date` deadline molle pour urgence visuelle (`due-soon` J-14, `overdue` après). Granularité par item : profil organisation OU navire individuel ; validation hors campagne autorisée (`campaign_id` NULLable). Permissions doubles : admin/staff(`manage_organizations`) côté admin, admin OU `users.organization_id === org.id` côté adhérent (réplique `handleUpsertFleet`). Snapshots immuables : chaque INSERT history fige un JSON post-update pour audits + diffs Y-o-Y. Bonus : `src/types/validation.ts` (types miroir) + `src/lib/validation-store.ts` (dual-mode squelette, retombe silencieusement en demo sans persistance localStorage). Documentation : `docs/VALIDATION-ANNUELLE-FEATURE.md` (411 lignes). **Frontend reporté en sessions 46+** (banner adhérent, `/espace-adherent/validation`, `/admin/campagnes`, dashboard Y-o-Y).) v2.35.0 (session 44 : audit critique 360° du site via 4 agents en parallèle (sources factuelles, code quality, UX/UI, expert maritime), ~120 findings dont ~17 critiques. **18 fixes safes/rapides appliqués** : a11y (prefers-reduced-motion global, contrast `--color-foreground-muted` light 5.8:1 + dark 5.2:1 fix le FAIL 2.9:1 d'AAA, skip-link « Aller au contenu principal », `scroll-behavior: smooth`, `aria-current="page"` sur Header + MobileNav, AnimatedNumber respecte reduced-motion via `startTransition`, Newsletter form `<label htmlFor>` + `focus-visible:ring`), sources (suppression code mort `src/data/stats.ts` qui contenait des chiffres faux 28/111/1364 — source de vérité = `memberStats` 27/212/1487, dilution « 100 % d'emploi à la sortie » sur 3 endroits → « Insertion professionnelle élevée » pour écarter risque DGCCRF / publicité trompeuse, liens Legifrance/IMO/OIT cliquables sur `/ssgm` et `/boite-a-outils`, refactor `SALARY_SOURCES` en `Array<{label, url?}>`, `LAST_UPDATED` CCN 3228 → Avril 2026), code (`SITE_VERSION` 2.17.0 → 2.34.0). Documentation complète des findings reportés dans `docs/AUDIT-CRITIQUE-2026.md` avec roadmap priorisée (6 critiques C1-C6 sous 2 sem, ~25 hautes sous 1 mois, ~30 moyennes sous 1 trim, ~15 basses backlog). v2.34.0 (session 43) : bouclage des itérations campagne `/ecoles-de-la-mer` + dette docs. Tests unitaires `src/data/__tests__/career-salary.test.ts` ajoutés (10 tests : structure CAREER_PATHS, libellés, jalons triés, terminaison « Retraite ENIM » à 55 ans, et `getStepAtAge` sur les 4 parcours pont/machine/service/polyvalent aux âges 17, 22, 30, 38, 55, 60 + clamps inférieur/supérieur) → vitest 272/272. Décision modélisation antenne ENSM Bastia tranchée : on conserve **1 marqueur LPM** unique (pas de 17ᵉ marqueur ENSM dédié), `SCHOOL_COUNTS.ensmSites` retourne dynamiquement 5 (4 ENSM + 1 LPM avec formation `officier`). Audit live des 12 URLs LPM toujours bloqué (sandbox sans réseau, `curl` HTTP 000 et `WebFetch` ECONNREFUSED, déjà constaté en session 42) — runbook ajouté dans `docs/CAMPAGNE-ECOLES-DE-LA-MER.md` § 4.1 pour exécution manuelle avec stratégie de remplacement (URL académique → fiche DAM → page récap GASPE). Runbook nettoyage CMS prod ajouté en § 7.8 (sections à vérifier : `map-intro` espace, `narrative-ensm-body` Parcoursup, `final-cta-bg-image` vide ou Media Library, avec commandes `wrangler d1 execute --remote` ciblées). Annexe historique enrichie sessions 42 + 43. v2.32.3 : audit a11y axe-core sur `/admin/adherents` + fixes (`aria-label` filtres, `Field2` implicit-label, `SortHeader` `aria-pressed`, AdminSidebar `--gaspe-neutral-400`, StatCard hint sans `/70`). v2.32.2 : tri + add demo + CSV /admin/adherents. v2.32.1 : Hydros publication auto admin. Migrations prod 0001-0026 appliquées, smoke tests 26 avr OK : 30 orgs actives, college A=23/B=4/C=3, social3228 True=26/False=3.) Documentation dédiée : `docs/CAMPAGNE-ECOLES-DE-LA-MER.md`.
...
- ⏳ Migration 0026 (session 40 : ALTER ADD `jobs.created_by` + index, idempotent via `::warning::` du workflow) – à appliquer au prochain merge main
- ⏳ Migration 0027 (session 45 : table `fleet_validation_campaigns` – cycle annuel draft/open/closed, target_year UNIQUE, target_date deadline molle, pas d'auto-fermeture) – à appliquer au prochain merge main
- ⏳ Migration 0028 (session 45 : ALTER ADD `last_validated_year/at/by` sur `organizations` + `organization_vessels` – cache projeté pour rendu rapide banner/dashboard, idempotent via `::warning::`) – à appliquer au prochain merge main
- ⏳ Migration 0029 (session 45 : table `validation_history` – trace immuable 1 ligne par item validé, snapshot_json post-update, `campaign_id` NULLable pour validation hors campagne) – à appliquer au prochain merge main
...
│   └── __tests__/         # Unit tests (319 tests, 27 files – +47 helpers validation annuelle session 45)
├── types/index.ts         # Centralized type re-exports
└── test/setup.ts          # Vitest test setup
workers/
├── api.ts                 # CF Worker: 76 endpoints (cf. tableau ci-dessous)
├── handlers/              # Helpers purs (session 45 : validation-helpers.ts)
...
└── migrations/            # 17 migrations totales
    ...
    ├── 0021_repair_alter_organizations_college.sql # repair college + social3228 (session 40)
    ├── 0022_repair_alter_users_suppleant.sql       # repair suppleant_user_id (session 40)
    ├── 0023_repair_alter_users_staff_permissions.sql # repair staff_permissions (session 40)
    ├── 0024_repair_alter_vessels_crew_by_brevet.sql # repair crew_by_brevet (session 40)
    ├── 0025_repair_data.sql              # repair data drift (session 40)
    ├── 0026_repair_alter_jobs_created_by.sql # repair jobs.created_by (session 40)
    ├── 0027_fleet_validation_campaigns.sql # Cycle annuel campagnes (session 45)
    ├── 0028_alter_validation_status.sql  # ALTER last_validated_* orgs + vessels (session 45)
    └── 0029_validation_history.sql       # Trace immuable validations (session 45)
```

## Worker API – 76 endpoints
...
| /api/campaigns | GET | JWT+admin/staff(orgs) |
| /api/campaigns | POST | JWT+admin/staff(orgs) |
| /api/campaigns/:id | PATCH | JWT+admin/staff(orgs) |
| /api/campaigns/:id/dashboard | GET | JWT+admin/staff(orgs) |
| /api/organizations/:slug/validations | GET | JWT+admin/staff/same-org |
| /api/organizations/:slug/validations | POST | JWT+admin/staff/same-org |

## Database (D1 – 24 tables, migrations 0001-0029 prêtes ; 0001-0025 appliquées en prod (vérifié 26 avr 2026 post-fix token), 0026-0029 (jobs.created_by, fleet_validation_campaigns, ALTER last_validated_*, validation_history) – à appliquer au prochain merge main)
| Table | Description |
|-------|-------------|
...
| 49 | 2.40.0 | **Documentation exhaustive + notifications email J+0 ouverture campagne**. (1) Refonte complète de la spec `docs/VALIDATION-ANNUELLE-FEATURE.md` : § 6 « Flux frontend » passe de stub session 45 à description détaillée des 5 surfaces livrées sessions 46-47 (banner dynamique avec `deriveUrgency()`, page `/espace-adherent/validation` avec ses 3 états par item, `/admin/campagnes` avec son `CreateCampaignForm`, dashboard `/admin/campagnes/detail?id=X` avec ses 4 summary cards et son `mailto:` BCC, tab démo non-interactive). § 9 « Évolutions futures » remis à jour avec section « Livré depuis la rédaction initiale » (manage_validations session 48, diff Y-o-Y session 47, notif J+0 session 49) + backlog clair pour les J-14 / J+0 deadline (nécessitent Cron Trigger), override admin, export PDF, validation partielle, webhook ADF/UMA. Nouvelle § 12 « Notification email J+0 ouverture campagne » (~80 lignes : vision, implémentation, template HTML, audience, reste à faire). Footer doc enrichi (sessions 45-49, RBAC + notif sections explicites). (2) Notifications email Brevo `J+0 ouverture campagne` : helper privé `notifyCampaignOpened(env, campaign)` ajouté à `workers/api.ts` (~140 lignes incluant le template HTML inline). Déclenché depuis `handleCreateCampaign` quand création directe en `status='open'` ET depuis `handleUpdateCampaign` à la transition `draft → open` (try/catch pour ne pas faire échouer le PATCH). Best-effort : no-op silencieux si `BREVO_API_KEY` absent (dev local), erreurs Brevo loggées mais ne propagent pas. Audience : `users.is_primary=1` + `archived=0` joints aux `organizations` actives. Pas de filtre college / 3228. Envoi séquentiel (rate-limit safety, < 30 destinataires en pratique). Helper `renderCampaignOpenedEmailHtml(params)` génère un email transactionnel charté ACF avec sanitization. Bump 2.39.0 → 2.40.0. **Qualité** : lint 0/0, tsc 0, vitest 333/333, build 117 pages OK. **TODO P3 résiduel** : J-14 / J+0 deadline (Cron Trigger), export PDF (lib jsPDF), smoke tests prod (sandbox sans réseau). |
| 48 | 2.39.0 | **Permission dédiée `manage_validations`** + documentation session 47. (1) Nouvelle permission staff `manage_validations` (11ᵉ permission, après `manage_agenda`). Couvre la gestion complète des campagnes (CRUD, dashboard, diff Y-o-Y). Découplée de `manage_organizations` qui restait coller un peu trop large. Code modifié : `src/lib/auth/types.ts` (ajout au type `StaffPermission` + libellé), `workers/api.ts` (4 endpoints campaigns passent de `manage_organizations` -> `manage_validations` ; `canActOnOrg` accepte les deux pour rétro-compatibilité), `AdminSidebar.tsx` (item « Campagnes validation » sur `manage_validations`). Pages campagnes admin restent sur `isStaffOrAdmin` côté front (cohérent avec /admin/votes etc., le Worker enforce). Schéma D1 inchangé (la permission est un simple string dans le JSON `users.staff_permissions`). (2) Documentation : ajout § 11 « Diff Y-o-Y (session 47) » dans `docs/VALIDATION-ANNUELLE-FEATURE.md` (~80 lignes : règles `diffSnapshots`, 14 tests, modal, intégration dashboard, dict `FIELD_LABELS_FR` 15 entrées). § 3 Permissions remis à jour avec le nouveau tableau. Footer doc enrichi : sessions 45-47, 8 helpers purs, 61 tests unitaires. Bump 2.38.0 -> 2.39.0. **Qualité** : lint 0/0, tsc 0, vitest 333/333 (inchangé), build 117 pages OK. **TODO P3 résiduel** : notifications email Brevo (J+0/J-14/J+0 deadline), export PDF d'attestation, smoke tests prod (sandbox sans réseau, post-merge auto). |
| 47 | 2.38.0 | **Vue diff Y-o-Y validation annuelle** (suite directe session 46). (1) Helper pur `diffSnapshots(before, after, options)` dans `workers/handlers/validation-helpers.ts` (~75 lignes) : deep equality avec `sortKeys` pour ignorer l'ordre des cles JSON, categorise chaque champ en `added`/`removed`/`modified`/`unchanged`, exclut `id` par defaut, tri canonique (modified > added > removed > unchanged) puis alphabetique. **14 nouveaux tests vitest** -> **333 tests verts** (319 + 14). (2) Duplication front `src/lib/validation-diff.ts` (workers/ exclu du tsconfig, evite l'import cross-tree) avec dict `FIELD_LABELS_FR` (15 libelles francais) + helper `formatDiffValue` (string/number/boolean/object). (3) Composant `<SnapshotDiffModal>` (`src/components/validation/`) : overlay z-50 fixed inset-0, fetch `listValidationsForOrg(slug)` au mount, filtre history sur `targetYear` et `targetYear - 1`, paire les items par `(itemType, itemId)`, calcule `diffSnapshots()` par paire, rendu en cartes avec tableau 4 colonnes (Champ / N-1 / N / Etat) + badges status colores. Tri profil > navires alpha, empty states (pas de N-1 = nouvel item, pas de N = item retire, aucun changement = badge vert). Fermeture clavier (Echap) + click backdrop + bouton fermer. (4) Colonne « Diff Y-o-Y » ajoutee au tableau `/admin/campagnes/detail?id=X` avec bouton « Voir le diff » par ligne (aria-label francais), state `diffOrg` controle le modal. Bump 2.37.0 -> 2.38.0. Build 117 pages, lint 0/0, tsc 0. **Reste TODO P3** : permission dediee `manage_validations`, notifications email Brevo (J+0 / J-14 / J+0 deadline), export PDF d'attestation, smoke tests prod (sandbox sans reseau). |
| 46 | 2.37.0 | **Frontend Validation annuelle** (suite session 45). 4 commits atomiques sur `main` (49ff399 → ce-commit). (1) **Banner adhérent** `src/components/validation/ValidationCampaignBanner.tsx` : détecte campagne ouverte via `listCampaigns().current`, fetch historique + flotte, calcule items pendings (profil + navires non encore validés pour `target_year`), affiche urgence visuelle teal/orange/red via `deriveUrgency()` (réplique côté front du helper Worker testé), CTA `/espace-adherent/validation`, silencieux en mode démo. Helper `ValidationCampaignBannerForUser({ companyName })` qui résout le slug via `members.find()`. Intégré dans `/espace-adherent` (dashboard) sous le bandeau suppléant. (2) **Page `/espace-adherent/validation`** : charge en parallèle (campagne courante + historique + flotte) ; carte profil avec sub-form `ProfileEditForm` éditable inline (email/phone/address/website/description/employeeCount/shipCount, fields liés au User connecté pour email/phone/address car `Member` n'expose pas ces champs) ; une carte par navire avec `FleetVesselForm` réutilisé pour édition inline ; 3 états par item : `pending` / `unchanged` / `edited` ; bouton « Tout marquer Inchangé » pour validation passive en masse ; bandeau sticky en bas avec compteur d'items prêts + submit ; refresh historique post-submit, reset état local. Empty state si pas de navires (lien `/espace-adherent/flotte`). Already-validated badge vert pour items déjà à jour. (3) **Page `/admin/campagnes`** + AdminSidebar entry « Campagnes validation » (icône `ClipboardCheckIcon`, sous Organisation, perm `manage_organizations`) : formulaire création `CreateCampaignForm` avec target_year (défaut année courante UTC), target_date (défaut 31 mars), notes, checkbox « Ouvrir immédiatement » (sinon brouillon) ; liste cartes par campagne avec badge urgence (draft/open/due-soon/overdue/closed) calculé via `deriveUrgency()` ; boutons contextuels « Ouvrir » (draft) / « Clôturer » (open) / « Tableau de bord » (open ou closed) → `/admin/campagnes/detail?id=X` en query param car static export ne supporte pas `dynamicParams=true` (pattern `/espace-adherent/votes/detail` session 38) ; `confirm()` avant chaque changement de statut ; empty state explicite + bandeau mode-démo si pas d'API. (4) **Dashboard `/admin/campagnes/detail?id=X`** (`CampaignDashboardClient.tsx`) : 4 summary cards (compagnies validées / en attente / navires / emails contacts) ; tableau filtré (Toutes/Pending/Validées avec compteurs dynamiques) et trié (compagnie/navires/état) ; sort headers cliquables avec `aria-pressed` + `aria-label` français explicite ; barre de progression visuelle par ligne (% navires validés) en teal ; bouton **« Relancer N retardataires »** : mailto BCC pré-rempli avec emails titulaires des compagnies non fully + subject + body français + lien direct vers `/espace-adherent/validation`, déduplication des emails. Liens : nom compagnie → fiche publique, email titulaire → mailto direct. (5) **Tab démo `validation`** sur `/decouvrir-espace-adherent` : 4 items fictifs (1 profil validé + 1 navire validé + 2 navires pending), bandeau campagne ouverte 2027, barre de progression % validés, boutons disabled (cohérent avec « démo non-interactive »), CTA adhésion. (6) **AdminSidebar** : nouvelle icône `ClipboardCheckIcon` SVG inline. (7) **Routes ajoutées** : `/admin/campagnes`, `/admin/campagnes/detail`, `/espace-adherent/validation`, tab démo. Build 114 → **117 pages**. (8) **Bump** : package.json + `SITE_VERSION` 2.36.0 → 2.37.0. **Qualité** : lint 0/0, tsc 0, vitest 319/319 (inchangé, pas de nouveaux helpers purs), build 117 pages. **Pas câblé cette session** : permission dédiée `manage_validations` (réutilisation de `manage_organizations` suffisante) ; vue diff Y-o-Y détaillée des snapshots (le tableau dashboard agrège, mais ne montre pas le diff champ par champ) ; export PDF d'attestation ; notifications email Brevo (J+0 ouverture, J-14 deadline, J+0 deadline). **Bloqueurs env (inchangés)** : sandbox sans réseau (smoke tests prod via merge auto). |
| 45 | 2.36.0 | **Backend Validation annuelle des données adhérents** (option 3 du brief : sous-tâches A+B groupées, puis C+D séparément). 6 commits atomiques sur `main` (97c0bbe → eb1b700). (1) **Docs** : nouveau `docs/VALIDATION-ANNUELLE-FEATURE.md` (411 lignes) — vision, modèle D1 détaillé, endpoints, permissions, 5 helpers + scénario utilisateur janvier→avril, sécurité/audit, évolutions futures, choix techniques (cache projeté `last_validated_*`, INTEGER autoincrement pour `campaigns.id`, pas de table `campaign_targets`, pas de trigger SQL). (2) **Migrations D1 0027-0029** : `fleet_validation_campaigns` (cycle `draft → open → closed`, `target_year` UNIQUE, deadline molle, pas d'auto-fermeture, FK `users(id)` TEXT) + ALTER `last_validated_year/at/by` sur `organizations` + `organization_vessels` (cache projeté pour requêtes chaud chemin) + `validation_history` (1 ligne par item, `snapshot_json` immuable, `campaign_id` NULLable, `is_unchanged` flag). Indexes : `(status)`, `(target_year)`, `(organization_id, target_year)`, `(item_type, item_id, target_year)`, `(campaign_id)`. (3) **Helpers purs** `workers/handlers/validation-helpers.ts` (~330 lignes, zéro dépendance D1) : `isItemValidatedForYear`, `countValidatedItems`, `deriveCampaignUrgency` (`nowMs` injectable, `dueSoonDays` configurable défaut J-14), `resolveTargetYear` (UTC, fallback année courante), `buildProfileSnapshot`, `buildVesselSnapshot` (parse défensif `crew_by_brevet` JSON, filtre valeurs ≤ 0, floor numeric), `parseValidationItems` (max 200 items, anti-doublons profile + vessel-id, `ValidationInputError` -> 400), `summarizeOrgForDashboard`. (4) **Endpoints Worker** (~640 lignes ajoutées en section dédiée fin de `workers/api.ts`) : 6 routes (`/api/campaigns` GET/POST + `/api/campaigns/:id` PATCH + `/api/campaigns/:id/dashboard` GET + `/api/organizations/:slug/validations` GET/POST). Permissions : admin/staff(`manage_organizations`) côté admin, admin OU `users.organization_id === org.id` côté adhérent (réplique `handleUpsertFleet`). Soumission atomique en 2 batchs D1 : (a) UPDATEs `organizations` + `organization_vessels` selon `data` fourni, (b) re-lecture post-update + INSERTs `validation_history` + UPDATEs `last_validated_*`. Side effects PATCH : `draft → open` set `opened_at`, `* → closed` set `closed_at`. `ensureValidationTables()` défensif (CREATE TABLE IF NOT EXISTS, pas d'ALTER en runtime car SQLite ne supporte pas IF NOT EXISTS sur ALTER). Total **76 endpoints Worker**. (5) **Tests vitest** : 47 nouveaux dans `workers/handlers/__tests__/validation-helpers.test.ts` couvrant edge cases nullables (last=null, =target, >target), comptes empty/all/none/percentage rounding, urgency draft/closed/no-date/malformed/before-threshold/at-threshold/in-window/overdue/custom-dueSoonDays, target year UTC, profile snapshot null vs undefined, vessel parse JSON valide/invalide/array/all-filtered/floor, parseValidationItems non-array/empty/>200/profile-only/vessel-only/mixed/invalid-type/missing-id/empty-id/duplicates/double-profile/non-object/unchanged-coercion/data-not-object, summarize fully/empty-vessels/missing-profile/partial/identity-fields. **319 tests verts** (272 baseline + 47, 27 fichiers). `vitest.config.ts` étendu pour inclure `workers/**/*.test.ts`. (6) **Bonus frontend (sans UI)** : `src/types/validation.ts` (10 types miroirs : `ValidationCampaign`, `ValidationHistoryEntry`, `OrgProfileSnapshot`, `VesselSnapshot`, `ValidationSubmitItem`, `ValidationSubmitResult`, `CampaignDashboard`, `CampaignDashboardOrgRow`, etc.) + `src/lib/validation-store.ts` (6 helpers async dual-mode `listCampaigns`/`createCampaign`/`updateCampaign`/`getCampaignDashboard`/`listValidationsForOrg`/`submitValidations`, retombe silencieusement en mode demo SANS persister localStorage pour ne pas masquer l'absence de stockage crédible). (7) **Routing** : nouvelles routes ajoutées avant le 404 final dans `fetch()` ; pas de collision avec `/api/organizations/:id` (regex `[^/]+$` exclut le `/validations`). (8) **Drift fix** : sync `package-lock.json` 2.33.1 → 2.36.0 (drift accumulé sessions 33-44). **Bump** package.json + `SITE_VERSION` 2.35.0 → 2.36.0. **Qualité** : lint 0/0, tsc 0, vitest **319/319**, build 114 pages. **Frontend reporté en sessions 46+** : banner adhérent dynamique sur `/espace-adherent` (urgence visuelle), page `/espace-adherent/validation` (cartes profil + navires + boutons inchangé/modifier), `/admin/campagnes` (liste + modal création + ouvrir/clôturer), dashboard `/admin/campagnes/:id` (breakdown + mailto BCC retardataires + diff Y-o-Y), tab démo. |
| 44 | 2.35.0 | **Audit critique 360° du site (4 agents parallèles)** + 18 fixes safes/rapides. (1) **Méthodo** : 4 agents `Explore` lancés simultanément — sources factuelles (35 violations), code quality (25 issues), UX/UI (35 findings), expert maritime (~27 issues). Total ~120 findings, ~17 critiques. Consolidation puis application sélective des items XS/S à fort impact, le reste documenté en roadmap priorisée. (2) **A11y appliqué** (WCAG 2.1 AA) : `prefers-reduced-motion` global dans `globals.css` (coupe scroll-reveal/marquee/gradient-shift), contrast `--color-foreground-muted` light 4.2:1 → 5.8:1 (`neutral-600` → `neutral-700`) + dark 2.9:1 (FAIL) → 5.2:1 (`#9E9893` → `#C4BDB5`), skip-link « Aller au contenu principal » dans root layout + `id="main-content"` sur `<main>` (public + admin), `scroll-behavior: smooth` sur `html`, `aria-current="page"` sur Header + MobileNav (3 endroits), AnimatedNumber respecte reduced-motion via `startTransition` (0 warning ESLint), Newsletter form `<label htmlFor>` + `focus-visible:ring-2 ring-offset` + `active:scale-95` sur bouton. (3) **Sources / factuel appliqué** : suppression `src/data/stats.ts` (code mort, 0 import, contenait 28/111/1364 — chiffres faux ; source de vérité = `memberStats` 27/212/1487), dilution « 100 % d'emploi à la sortie » sur 3 endroits (`/ecoles-de-la-mer` ×2 + `cms-defaults.ts` ×1) → « Insertion professionnelle élevée » pour écarter risque DGCCRF / publicité trompeuse, liens Legifrance / IMO / OIT cliquables sur `/ssgm` (décret 2015-1575, articles L5521-1 à L5521-4, STCW 1978+amendments 2010, MLC 2006 ratifiée 28/02/2013), refactor `SALARY_SOURCES` CCN 3228 en `Array<{label, url?}>` avec liens (Legifrance KALICONT, ENIM), affichage `/boite-a-outils` mis à jour pour rendre les liens cliquables (decoration-dotted), `LAST_UPDATED` CCN 3228 « Janvier 2026 » → « Avril 2026 » (alignement avec barème NAO 2026 + indemnités annexe 1). (4) **Code appliqué** : `SITE_VERSION` 2.17.0 → 2.34.0 → 2.35.0 (drift de version résolu après 17 releases, commentaire ajouté pour rappeler sync `package.json`). (5) **Documentation** : nouveau `docs/AUDIT-CRITIQUE-2026.md` (~370 lignes) avec roadmap priorisée — 6 critiques C1-C6 (NAO sans n° avenant JO, stats sans millésime, EU ETS / FuelEU absents, ENIM sans attestation, ENTEC 2005 obsolète, pas de date MAJ visible) + ~25 hautes (UMS/GT, PSL/OST, ADF positionnement, modals sans ESC, Worker monolithique, pages admin > 1000 lignes, AdemeSimulator extraction, useSafeLocalStorage, Zod client validation, target=_blank, etc.) + ~30 moyennes + ~15 basses. **Bump** package.json v2.34.0 → v2.35.0 + CLAUDE.md ligne 6 (114 pages OK). **Qualité** : lint 0/0, tsc 0, vitest 272/272, build 114 pages. **Tous les items reportés sont safes par construction** : la session a appliqué uniquement ce qui ne pouvait pas régresser. |
