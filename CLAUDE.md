...
- **Version**: v2.32.0 (session 40 suite, PR #68 : page admin de fusion `/admin/adherents` qui remplace `/admin/organisations` + `/admin/membres`. Listing API-first hérité de `/admin/organisations` avec contacts inline (table embarquée par compagnie), modal CRUD complet hérité d'`/admin/membres`, filtres collège A/B/C + CCN 3228 + cotisation + statut + recherche, vue toggle Détaillée/Tableau, archivage/restauration via PATCH. Anciennes routes redirigent vers la nouvelle. memberSchema/StoredMember étendus pour persister email/phone/address/membershipStatus en mode demo. v2.31.0 conservait : Vote UX type-spécifique – éditeur d'options multi-items + date picker + drag-and-drop ranking. Migrations prod 0001-0026 appliquées, smoke tests 26 avr OK : 30 orgs actives, college A=23/B=4/C=3, social3228 True=26/False=3.)
