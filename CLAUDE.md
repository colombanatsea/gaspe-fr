...
- **Version**: v2.32.2 (session 41 Lot 2 : UX `/admin/adherents`. (1) **Tri colonnes** dans la vue Tableau — 8 en-têtes `SortHeader` cliquables (Compagnie / Ville / Région / Catégorie / Coll. / 3228 / Cotisation / Contacts) avec indicateur ▲/▼ et toggle asc↔desc. `aria-sort` porté par un wrapper `role="columnheader"`. (2) **Bouton « Ajouter un adhérent »** affiché en mode demo uniquement (caché si `isApiMode()`) — ouvre `EditModal` en mode `create` avec form vide. Helper `addMember(member)` ajouté à `members-store.ts` (demo only, refuse les slugs en doublon). En mode prod : message « Création via seed members.ts + redéploy ». (3) **Export CSV** de la liste filtrée — bouton header, format Excel FR (`;` + UTF-8 BOM), 19 colonnes incluant collège, social3228, cotisation, contact responsable. v2.32.1 : Hydros publication auto sur `/admin/offres/new`. v2.32.0 : page admin de fusion `/admin/adherents`. Migrations prod 0001-0026 appliquées, smoke tests 26 avr OK : 30 orgs actives, college A=23/B=4/C=3, social3228 True=26/False=3.)
