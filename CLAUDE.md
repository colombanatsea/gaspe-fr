...
- **Version**: v2.33.1 (session 42 : landing `/ecoles-de-la-mer` – campagne « Les écoles de la mer » destinée aux 14-25 ans (entonnoir QR codes affiches navires/gares maritimes → `/nos-compagnies-recrutent`). v2.33.0 (PR #73) : 6 sections narratives (hero + quiz orientation Pont/Machine/Service 6 questions + récit LPM/ENSM + carte Leaflet 12 LPM + 4 sites ENSM + simulateur carrière 17-45 ans / 4 parcours / barème CCN 3228 + CTA), 16 `EducationalOrganization` JSON-LD pour SEO local, 12 chaînes user-facing CMS-éditables, +4 mots-clés (`marin de commerce`, `lycée maritime`, `école nationale supérieure maritime`, `formation maritime jeune`), +8 unit tests `quiz-scoring`. v2.33.1 (PR #74) : itérations post go-live – hero photo (`/campagne/ecoles-de-la-mer-hero.jpg`, next/image fill priority unoptimized + overlay foreground gradient, fallback silencieux), ENSM clarifiée comme **une école unique répartie sur 5 sites** (4 historiques Le Havre / Marseille / Nantes / Saint-Malo + antenne récente au LPM Bastia, plus jamais « 4 écoles »), LPM Bastia gagne le brevet officier (« tu peux y faire toute ta scolarité, du CAP au brevet officier, sans quitter Bastia »), retrait de « Écoles de la mer » de `mainNavigation` (page volontairement absente du header — atteinte par QR code, footer et sitemap), **timeline carrière accélérée** (PATH_PONT 23→22, 30→26, 38→30, 45→38 ; PATH_MACHINE 24→22, 31→27, 38→30, 45→38 ; PATH_SERVICE 26→24, 35→30, 45→40 ; PATH_OFFICIER 24→23, 30→26, 40→30, 45→38) — titre simulateur « À 17 ans tu commences. À 30 ans tu commandes. », CTA final « {compagnies} compagnies maritimes adhérentes du GASPE recrutent partout dans l'hexagone et les outremer. » (placeholder dynamique via `memberStats.compagnies`), `public/campagne/README.md` runbook upload image. Bump 2.32.3 → 2.33.1 ; **262 tests verts** (+8), lint 0/0, tsc 0, build 114 pages. v2.32.3 (session 41 Lot 3 : audit a11y axe-core sur `/admin/adherents` + fixes. Nouveau spec `e2e/a11y-admin-adherents.spec.ts` (3 tests : vue Détaillée, vue Tableau, modal création) — 0 violation critical/serious sur WCAG 2.1 AA. Fixes : (1) `aria-label` sur les 6 inputs de la barre de filtres (search + 5 selects) — résout `select-name` critical. (2) `Field2` enveloppe les enfants dans un `<label>` (implicit-label) — résout les 17 selects/inputs sans label dans la modal. (3) `SortHeader` : `role="columnheader"` retiré (manquait le parent `role="row"`/`role="grid"`), remplacé par `aria-pressed` + `aria-label` enrichi indiquant la direction du tri. (4) AdminSidebar : `--gaspe-neutral-500` → `--gaspe-neutral-400` sur 5 occurrences (titres sections, email user, logout, "Voir le site") — passe le ratio AA `#B0A89E` vs `#222221` ≈ 6.6:1. (5) StatCard hint : retrait de l'opacité `/70` (3.1:1 → 5.5:1). Dépendance dev ajoutée : `@axe-core/playwright`. v2.32.2 : tri + add demo + CSV /admin/adherents. v2.32.1 : Hydros publication auto admin. Migrations prod 0001-0026 appliquées, smoke tests 26 avr OK : 30 orgs actives, college A=23/B=4/C=3, social3228 True=26/False=3.)
...
src/
├── app/
│   ├── (public)/          # 36 routes publiques (+ positions/[slug], /actualites refont, /feed.xml, /espace-adherent/annuaire-flotte session 38, /ecoles-de-la-mer session 42)
...
│   ├── schools/           # OrientationQuiz, SchoolMap (Leaflet lazy-loaded), CareerSimulator – campagne /ecoles-de-la-mer (session 42)
...
├── data/                  # Static data (members, jobs, ccn3228, stcw, formations, ssgm, navigation, stats, routes, maritime-certifications, positions, fleet-seed, schools/quiz-questions/career-salary session 42)
├── lib/
...
| 42 (suite) | 2.33.1 | **PR #74 — `/ecoles-de-la-mer` itérations post go-live**. Retours éditeur après mise en ligne de la v2.33.0. (1) **Hero background photo** : `/campagne/ecoles-de-la-mer-hero.jpg` (492 KB, salle des machines, kit affiches v2 du studio créatif) chargé via `next/image` `fill priority unoptimized` (compatible static export Cloudflare Pages), overlay foreground en dégradé `from-black/70 via-black/40 to-black/60` pour préserver la lisibilité du texte ; chemin éditable depuis le CMS via la section `hero-bg-image` ; fallback silencieux si le fichier est absent. (2) **ENSM = une école unique** : refonte du discours pour ne plus jamais parler de « 4 écoles ENSM » (erreur factuelle). L'ENSM est **une seule** école nationale supérieure maritime, répartie sur **4 sites historiques** (Le Havre = siège, Marseille, Nantes, Saint-Malo) + **1 antenne récente** au LPM Bastia → 5 lieux pour devenir officier de la marine marchande. Hero stats : « 1 école nationale supérieure (5 sites) ». Section carte titre : « 12 LPM + 1 ENSM » + sous-titre « 5 sites pour devenir officier, dont l'antenne récente au LPM Bastia ». Compteur sous la carte conditionné selon le filtre actif (`kind=lpm` → « 12 LPM », `kind=ensm` → « 5 sites », sinon « 17 sites »). Helper `SCHOOL_COUNTS.ensmSites` (`src/data/schools.ts:422`) calcule dynamiquement 4 sites ENSM + 1 LPM avec brevet officier. (3) **LPM Bastia gagne le brevet officier** : ajout de `ENSM_OFFICIER_CHEF` dans ses formations + description « Le seul site maritime de Méditerranée à former aux métiers du transport de passagers entre Corse et continent. L'ENSM y a installé une antenne : tu peux y faire toute ta scolarité, du CAP au brevet d'officier, sans quitter Bastia. ». (4) **Retrait du header public** : `« Écoles de la mer »` retirée de `mainNavigation` (`src/data/navigation.ts:3-7`) — la page est conçue pour les jeunes scannant un QR code sur les affiches de la campagne, pas pour un visiteur du site naviguant depuis l'accueil. **Conservée** dans le footer (`bottomLinks`), le sitemap et `metaFromPageId` pour SEO. (5) **Timeline carrière accélérée** : suite à un retour terrain (jeunes capitaines de ferry à 30 ans, chefs mécaniciens à 30 ans), réécriture de `src/data/career-salary.ts` — `PATH_PONT` 23→22 / 30→26 / 38→30 / 45→38 ; `PATH_MACHINE` 24→22 / 31→27 / 38→30 / 45→38 ; `PATH_SERVICE` 26→24 / 35→30 / 45→40 ; `PATH_OFFICIER` 24→23 / 30→26 / 40→30 / 45→38. Titre du simulateur : « À 17 ans tu commences. À 30 ans tu commandes. ». (6) **Final CTA dynamique** : `« {compagnies} compagnies maritimes adhérentes du GASPE recrutent partout dans l'hexagone et les outremer. »` — placeholder `{compagnies}` interpolé via `stats-placeholders.ts` (= 27 d'après `memberStats`). (7) **`public/campagne/README.md`** : runbook complet d'upload de l'image de fond (chemin attendu, procédure, optimisation JPEG < 250 KB, crédits, personnalisation CMS). **Qualité** : 262 tests verts, lint 0/0, tsc 0, build 114 pages OK. |
| 42 | 2.33.0 | **PR #73 — Landing `/ecoles-de-la-mer`** (campagne « Les écoles de la mer »). **Contexte** : campagne d'affichage déployée à bord des navires GASPE et dans les gares maritimes pour orienter les **14-25 ans** vers les métiers de marin de commerce. Les affiches portent un **QR code** qui pointe vers cette page, puis l'utilisateur est redirigé en fin d'entonnoir vers `/nos-compagnies-recrutent`. **6 sections narratives en scroll** : (1) Hero plein-écran avec eyebrow « Les écoles de la mer », titre « Ton bureau ? Il tangue. Et il paie. », sous-titre « 100% d'emploi à la sortie. Tu pilotes. Tu répares. Tu navigues. Dès 14 ans. », 2 CTA (Quiz, Trouver mon école). (2) Quiz d'orientation **Pont / Machine / Service** : 6 questions (`src/data/quiz-questions.ts`, 200 lignes), scoring pure function `src/lib/quiz-scoring.ts` avec **8 unit tests** (`src/lib/__tests__/quiz-scoring.test.ts`, 106 lignes), recommandations de formations en sortie, composant `OrientationQuiz` (240 lignes). (3) Section narrative LPM/ENSM (titre + body éditables CMS). (4) **Carte interactive Leaflet** des 12 LPM + 4 sites ENSM (`SchoolMap`, 191 lignes, lazy-loaded via `next/dynamic` `ssr: false`) avec filtres `kind` (lpm/ensm) et `level` (cap/bac_pro/bts/post_bac/officier), popup avec lien site web. (5) **Simulateur de carrière** (`CareerSimulator`, 142 lignes) : slider d'âge 17-45 ans, **4 parcours** (`PATH_PONT`, `PATH_MACHINE`, `PATH_SERVICE`, `PATH_OFFICIER` ENSM) avec jalons salariaux issus de la grille CCN 3228 (`src/data/career-salary.ts`, 241 lignes). (6) CTA final → `/nos-compagnies-recrutent`. **SEO** : 16 `EducationalOrganization` JSON-LD (un par école, boost SEO local), nouveau composant `EducationalOrganizationJsonLd` dans `src/components/shared/SEOJsonLd.tsx`, entrée `ecoles-de-la-mer` dans `DEFAULT_PAGE_META` (`src/lib/seo.ts:175-190`) avec 8 keywords ciblés, 4 nouveaux termes ajoutés à `SITE_KEYWORDS` : `marin de commerce`, `lycée maritime`, `école nationale supérieure maritime`, `formation maritime jeune`. **Données partenaires** : registre complet des 12 LPM + 4 sites ENSM dans `src/data/schools.ts` (436 lignes) — types `SchoolKind` (lpm/ensm), `SchoolLevel` (cap/bac_pro/bts/post_bac/officier), `SchoolFamily` (pont/machine/service/polyvalent, aligné sur les 17 brevets CCN 3228), helpers `getLpmList`, `getEnsmSites`, `getSchoolBySlug`, `getSchoolsByLevel`, `getSchoolsByFamily`, `SCHOOL_COUNTS`, `LEVEL_LABELS`, `FAMILY_LABELS`. Sources : DAM 2026 + supmaritime.fr. **CMS** : 12 chaînes user-facing éditables via `/admin/pages` → page « Écoles de la mer » (`page-header-title/description`, `hero-bg-image/eyebrow/title/subtitle/cta-primary/cta-secondary`, `narrative-lpm-title/body`, `narrative-ensm-title/body`, `map-intro`, `final-cta-title/subtitle`). **Nav** : entrée ajoutée au top nav + footer + sitemap + `metaFromPageId`. **Total** : +9 nouveaux fichiers, +5 fichiers édités, +2349 lignes / -1 ligne. **Qualité** : 262 tests verts (+8), 0 warning ESLint, 0 erreur tsc, build OK 114 pages. |
