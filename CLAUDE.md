...
- **Version**: v2.22.2 (flotte éditable + seed 110 navires en prod · grille NAO 2026 alignée 17 classifications · sweep em-dashes typographique · correctifs post-flotte v2.22.1)
...
- Job offers in `src/data/jobs.ts` (4 offres Karu'Ferry / Step Group depuis session 33d – mise en main éditoriale admin via `/admin/jobs`) + dual-mode store (localStorage/D1)
- Employer guides in `src/data/ccn3228.ts` (10 guides + **17 classifications NAO 2026** alignées via `salaryGridRef` : Patron vedette <50 UMS, Capitaine 50–200 / 200–500 / 500–3000 / >3000 UMS + illimité, Chef Mécanicien 750 kW / 3000 kW / 8000 kW / illimité, Second capitaine, Commissaire de bord, etc. – chaque entrée mappée sur sa ligne de grille NAO 2026 Legifrance)
...
| 35b | 2.22.2 | **Correctifs post-déploiement flotte** (5 commits consolidés, PR merged sur main). **Migration 0013 appliquée en prod** (seed 110 navires via `INSERT OR IGNORE` idempotent + resolution de `organization_id` via subquery sur slug pour robustesse). **Migration 0014** : archive `keolis-bordeaux-metropole` côté D1 (la compagnie a été retirée de `members.ts` session 34 mais la table `organizations` gardait les 31 orgs du seed 0003 → `/notre-groupement` en mode API affichait 31, `memberStats` affichait 30 ; archive = UPDATE `archived=1` idempotent, le Worker filtre déjà `archived != 1` côté public). **Grille NAO 2026 alignée (source de vérité CCN 3228)** : `CLASSIFICATION_LEVELS` refondu 11 → **17 entrées** (ajouts Chef Mécanicien 750 kW / 3000 kW / 8000 kW / illimité, Capitaine 3000 UMS / illimité, Patron vedette <50 UMS borné, Commissaire de bord). Chaque classification acquiert un champ `salaryGridRef` qui mappe exactement vers la ligne de `SALARY_GRID_NAO_2026`. Labels Chef mécanicien bornés par **tranche UMS** (`<200 / 200–500 / 500–3000 / >3000 UMS`) et non plus par kW (les kW restent dans `requiredCertificates`). Simulateur `/boite-a-outils` : bandeau pédagogique (fonction + jauge UMS fixent la paie, pas le brevet détenu), "Estimation salariale" → **"Minima conventionnels"**, labels renommés (`brut mensuel minimum`, `net minimum estimé`). Sources explicitées : Avenant NAO 2026, Legifrance, L2253-1 C. trav., Code des transports, barèmes ENIM. **Typographie** : sweep global em-dashes `—` → en-dashes `–` sur 120 fichiers (src, workers, docs) pour conformité règle CLAUDE.md. **MemberMap homepage / `/nos-adherents`** : cadrage hexagone via `map.fitBounds(METROPOLE_BOUNDS)` au chargement (vs zoom fixe 6) → pas de débord mer du Nord sur grands écrans. **4 correctifs v2.22.1** : (1) logo `image12.png` réaffecté de Capstan Avocats vers **Compagnie Maritime DNO (Manche Iles Express)** ; (2) kit presse `/positions` – boutons "Télécharger le logo" (direct `/assets/brand/logo-gaspe.png`) + "Kit presse complet" (mailto contact@gaspe.fr) câblés ; (3) `useScrollReveal` – ajout d'un `MutationObserver` qui observe les `.reveal` ajoutés dynamiquement → fix onglets CCN 3228 qui restaient vides après switch (`Salaires`, `Aides`, `Formations`, `ENIM`, `STCW`) ; (4) header count 31 → 30 adhérents via migration 0014. **Qualité** : **249 tests verts**, 0 warning ESLint, 0 erreur tsc, build OK. **Hors scope résiduel** : population flotte Jalilo (pas de donnée remontée), compression vidéo ACF (ffmpeg absent env), Lighthouse réel (Chrome absent env), activation Brevo prod (secrets admin requis). |
