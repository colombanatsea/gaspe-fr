...
- **Version**: v2.30.4 (session 40 PR #67 : repair partielle migration 0005 – `ALTER TABLE jobs ADD COLUMN created_by` + recreate index `idx_jobs_created_by`. Cause : la table `jobs` avait été créée à un stade antérieur sans `created_by` ; le `CREATE INDEX ... ON jobs(created_by)` de 0005 plantait à chaque re-run. Détecté grâce au hardening de PR #65 + correction du token Cloudflare API par le user (D1:Edit + R2 Storage:Edit ajoutés à la mi-session 40). Smoke tests 26 avr 2026 désormais 8/9 verts ; staffPermissions absent du JSON est attendu — colonne NULL tant qu'aucun user n'est promu staff via /admin/comptes.)
