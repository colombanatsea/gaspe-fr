...
|---|---------|---------|--------|
| C1 | **NAO 2026 sans n° d'avenant ni publication JO** : la grille est correcte mais le site doit citer le numéro IDCC d'avenant et la date d'arrêté d'extension | `src/data/ccn3228.ts` § SALARY_SOURCES | XS — vérifier auprès du GASPE |
| C2 | **Stats passagers / véhicules sans millésime** : « 25 M+ passagers » sans année (probablement pré-COVID). À sourcer à partir des déclarations annuelles des adhérents. | `src/data/cms-defaults.ts` quick-stats | S |
| C3 | **EU ETS Maritime / FuelEU Maritime absents** alors qu'entrés en vigueur 2024 et touchent les adhérents directement. Prévoir une section dédiée sur `/transition-ecologique` ou `/positions`. | nouveau contenu | M |
| C4 | **Cotisations ENIM 2025/2026 sans attestation** : taux affichés sans lien vers caisse-gens-de-mer.fr ni note sur la classe AT/MP variable par entreprise. | `src/data/ccn3228.ts:506-514` | XS |
| C5 | **Facteurs CO₂ ENTEC 2005 obsolètes** dans AdemeSimulator (étude vieille de 21 ans). Migrer vers IMO GHG Study 2020 / 2023. | `src/components/simulator/AdemeSimulator.tsx:460` | M |
| C6 | **Aucune date « dernière vérification » visible** sur les datasets institutionnels (members, stats, fleet). Pas de traçabilité = crédibilité nulle. | global | S |
