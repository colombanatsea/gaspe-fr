...
// ── Sanitize HTML riche (preserve les tags inline / structure, strip XSS) ──
// Utilisé pour les champs `content` / `description` riches stockés en D1
// (CMS, formations, positions). Réplique de src/lib/sanitize-html.ts côté Worker
// (le Worker n'importe pas src/).
...
// ═══════════════════════════════════════════════════════════
//  CMS custom sections – Phase 1 hybride (migration 0042)
//
//  Permet à l'admin d'ajouter une section éditable sur une page système
//  sans toucher au code. Voir docs/CMS-HYBRID-PLAN.md.
// ═══════════════════════════════════════════════════════════
 ...
 */
async function handleCmsListAllCustomSections(
  env: Env,
  corsHeaders: Record<string, string>,
) {
  try {
 ...
 */
async function handleCmsCreateCustomSection(
  request: Request,
  env: Env,
  corsHeaders: Record<string, string>,
  pageId: string,
 ...
 */
async function handleCmsDeleteCustomSection(
  request: Request,
  env: Env,
  corsHeaders: Record<string, string>,
  pageId: string,
...
// ═══════════════════════════════════════════════════════════
//  CMS custom pages – Phase 3 hybride (migration 0043)
//
//  Pages entièrement custom (slug, label, description, content HTML),
//  publiées sous /p?slug=X. Distinctes des pages système (PAGE_DEFINITIONS
//  + templates dédiés) et des sections custom Phase 1 (ajoutées sur les
//  pages système).
// ═══════════════════════════════════════════════════════════
 ...
 */
async function handleCmsListCustomPages(
  request: Request,
  env: Env,
  corsHeaders: Record<string, string>,
) {
...
/**
 * GET /api/cms/custom-pages/:slug — détail d'une page custom.
 *   - Sans auth : 404 si non publiée ou archivée.
 *   - Avec auth admin : retourne aussi les brouillons.
 */
async function handleCmsGetCustomPage(
  env: Env,
  corsHeaders: Record<string, string>,
  slug: string,
) {
  ...
  try {
    // Public : on filtre published + non-archived. Le canal admin
    // utilise ?all=1 sur le list ; pour l'instant pas de bypass admin
    // sur le GET single (simple et suffisant pour la Phase 3 MVP).
    const row = await env.DB.prepare(
 ...
 * Body : { slug, label, description?, content?, published? }.
 */
async function handleCmsCreateCustomPage(
  request: Request,
  env: Env,
  corsHeaders: Record<string, string>,
) {
...
/**
 * PUT /api/cms/custom-pages/:slug — mettre à jour une page. Admin only.
 * Body : champs optionnels { label, description, content, published }.
 * Le slug n'est pas modifiable (permalien stable).
 */
async function handleCmsUpdateCustomPage(
  request: Request,
  env: Env,
  corsHeaders: Record<string, string>,
  slug: string,
  ...
  // Suppress unused var warning for auth (audit log peut être ajouté ici).
  void auth;
...
/**
 * DELETE /api/cms/custom-pages/:slug — soft-delete (is_archived = 1).
 * Admin only.
 */
async function handleCmsDeleteCustomPage(
  request: Request,
  env: Env,
  corsHeaders: Record<string, string>,
  slug: string,
...
// ════════════════════════════════════════════════════════════════════
//  Positions – CRUD (session 54, P0-2 PRODUCTION-SAFETY-2026)
//  Migration 0032_positions.sql.
//  Permission staff requise pour POST/PATCH/DELETE : `manage_positions`.
// ════════════════════════════════════════════════════════════════════
...
function toFrontendPosition(row: DbPosition) {
  return {
    id: row.id,
    slug: row.slug,
    title: row.title,
...
async function ensurePositionsTable(env: Env): Promise<void> {
  await env.DB.prepare(`
...
async function handlePositionsList(request: Request, env: Env, corsHeaders: Record<string, string>) {
...
async function handleCreatePosition(request: Request, env: Env, corsHeaders: Record<string, string>) {
